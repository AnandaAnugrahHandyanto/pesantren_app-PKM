        ...
        <main class="relative z-10 mx-auto flex min-h-[calc(100vh-4rem)] max-w-6xl flex-col items-center justify-center px-4 py-16 text-center sm:px-6">
            ...
            <div class="mb-2 select-none">
                <span class="font-extrabold leading-none tracking-tighter"
                      style="font-size: clamp(8rem, 25vw, 20rem); background: linear-gradient(to right, #67e8f9, #93c5fd, #a5b4fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
                    404
                </span>
            </div>
            ...
            <div class="mb-6 flex justify-center">
                <div class="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/20 bg-gradient-to-br from-red-500/20 to-pink-500/20 shadow-xl shadow-red-500/10 backdrop-blur-xl">
            ...
            <p class="mt-4 max-w-md text-sm leading-relaxed text-white/50 sm:text-base">
                Halaman yang kamu cari tidak ada, sudah dipindahkan, atau mungkin alamat URL-nya salah ketik.
            </p>

            {{-- Divider --}}
            <div class="my-8 h-px w-24 bg-gradient-to-r from-transparent via-white/20 to-transparent"></div>
            ...
            <div class="mt-12 flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
                <span class="text-xs text-white/30">Mungkin kamu mencari:</span>
                @auth
                    <a href="{{ url('/dashboard') }}" class="text-xs text-cyan-400/70 transition hover:text-cyan-300">Dashboard</a>
                    <a href="{{ url('/absensi') }}" class="text-xs text-cyan-400/70 transition hover:text-cyan-300">Absensi</a>
                    <a href="{{ url('/siswa') }}" class="text-xs text-cyan-400/70 transition hover:text-cyan-300">Data Siswa</a>
                    <a href="{{ url('/laporan') }}" class="text-xs text-cyan-400/70 transition hover:text-cyan-300">Laporan</a>
                @else
                    <a href="{{ route('login') }}" class="text-xs text-cyan-400/70 transition hover:text-cyan-300">Masuk</a>
                    <a href="{{ route('register') }}" class="text-xs text-cyan-400/70 transition hover:text-cyan-300">Daftar</a>
                @endauth
            </div>
