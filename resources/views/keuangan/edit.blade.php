            ...
            <form action="{{ route('keuangan.update', $keuangan) }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $keuangan->tanggal->toDateString()) }}" required
                    ...
                    <div>
                        <label for="jenis" class="mb-1 block text-sm font-medium text-white/80">Jenis <span aria-hidden="true">*</span></label>
                        <div class="relative">
                            <select id="jenis" name="jenis" required
                                ...
                                <option value="" disabled class="bg-indigo-950 text-white">Pilih jenis</option>
                                <option value="pemasukan" {{ old('jenis', $keuangan->jenis) === 'pemasukan' ? 'selected' : '' }} class="bg-indigo-950 text-white">Pemasukan</option>
                                <option value="pengeluaran" {{ old('jenis', $keuangan->jenis) === 'pengeluaran' ? 'selected' : '' }} class="bg-indigo-950 text-white">Pengeluaran</option>
                            </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="kategori" name="kategori" value="{{ old('kategori', $keuangan->kategori) }}"
                    ...
                    <div>
                        <label for="jumlah" class="mb-1 block text-sm font-medium text-white/80">Jumlah (Rp) <span aria-hidden="true">*</span></label>
                        <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah', $keuangan->jumlah) }}" min="0" step="0.01" required
                            class="w-full rounded-lg border border-white/30 bg-white/10 px-3 py-2 text-sm text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                        @error('jumlah') <p class="mt-1 text-xs text-red-300">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="siswa_id" class="mb-1 block text-sm font-medium text-white/80">Siswa (opsional)</label>
                        <div class="relative">
                            <select id="siswa_id" name="siswa_id"
                                class="w-full appearance-none rounded-lg border border-white/30 bg-white/10 px-3 py-2 pr-10 text-sm text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                                <option value="" {{ old('siswa_id', $keuangan->siswa_id) ? '' : 'selected' }} class="bg-indigo-950 text-white">- Tidak terkait siswa -</option>
                                @foreach ($siswas as $siswa)
                                    <option value="{{ $siswa->id }}" {{ old('siswa_id', $keuangan->siswa_id) == $siswa->id ? 'selected' : '' }} class="bg-indigo-950 text-white">{{ $siswa->nis }} - {{ $siswa->nama_lengkap }}</option>
                                @endforeach
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                <svg class="h-4 w-4 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div>
                    <label for="keterangan" class="mb-1 block text-sm font-medium text-white/80">Keterangan</label>
                    <textarea id="keterangan" name="keterangan" rows="3"
                        class="w-full rounded-lg border border-white/30 bg-white/10 px-3 py-2 text-sm text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">{{ old('keterangan', $keuangan->keterangan) }}</textarea>
