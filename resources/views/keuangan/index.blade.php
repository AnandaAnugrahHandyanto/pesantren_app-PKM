        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div class="stat-card">
                <div class="stat-icon bg-emerald-500/20">
                    <svg class="h-5 w-5 text-emerald-500 dark:text-emerald-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                ...
                <p class="stat-value text-emerald-700 dark:text-emerald-300">Rp {{ number_format($totalPemasukan, 0, ',', '.') }}</p>
                <p class="stat-sub">{{ number_format($countPemasukan) }} transaksi</p>
            </div>

            <div class="stat-card">
                <div class="stat-icon bg-rose-500/20">
                    <svg class="h-5 w-5 text-rose-500 dark:text-rose-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <p class="stat-label">Pengeluaran</p>
                <p class="stat-value text-rose-700 dark:text-rose-300">Rp {{ number_format($totalPengeluaran, 0, ',', '.') }}</p>
                <p class="stat-sub">{{ number_format($countPengeluaran) }} transaksi</p>
            </div>

            <div class="stat-card">
                <div class="stat-icon {{ $saldo >= 0 ? 'bg-blue-500/20' : 'bg-red-500/20' }}">
                    <svg class="h-5 w-5 {{ $saldo >= 0 ? 'text-blue-500 dark:text-blue-300' : 'text-red-500 dark:text-red-300' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                ...
                <p class="stat-value {{ $saldo >= 0 ? 'text-blue-700 dark:text-blue-300' : 'text-rose-700 dark:text-rose-300' }}">Rp {{ number_format(abs($saldo), 0, ',', '.') }}</p>
                <p class="stat-sub">{{ $saldo >= 0 ? 'Positif' : 'Defisit' }}</p>
            </div>

            <div class="stat-card">
                <div class="stat-icon bg-slate-50 dark:bg-white/10">
                    <svg class="h-5 w-5 text-slate-700 dark:text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <div class="content-card">
            ...
            <form method="GET" action="{{ route('keuangan.index') }}" class="flex flex-wrap items-end gap-3">
                ...
                @if (request()->anyFilled(['bulan', 'tahun', 'jenis']))
                    <a href="{{ route('keuangan.index') }}" class="btn-secondary !p-2">
                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <table>
                ...
                <tbody>
                    @forelse ($keuangans as $k)
                        <tr>
                            <td class="text-slate-900 dark:text-slate-700 dark:text-white/80 whitespace-nowrap">{{ $k->tanggal->format('d M Y') }}</td>
                            <td>
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold bg-rose-100 text-rose-800 dark:bg-rose-500/20 dark:text-rose-300">
                                        <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                        Pengeluaran
                            ...
                            <td class="text-right font-medium whitespace-nowrap {{ $k->jenis === 'pemasukan' ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300' }}">
                            ...
                            <td class="text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('keuangan.destroy', $k) }}" method="POST" class="inline"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                                class="rounded-lg border border-red-500/50 bg-red-500/10 px-3 py-1.5 text-xs font-semibold text-red-600 dark:text-red-300 backdrop-blur-sm transition hover:bg-red-500/20 hover:border-red-500/60">
                                            Hapus
                                        </button>
                                    </form>
                                </div>
