            ...
            <form action="{{ route('keuangan.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="jenis" name="jenis" required
                                ...
                                <option value="" disabled {{ old('jenis') ? '' : 'selected' }} class="bg-indigo-950 text-white">Pilih jenis</option>
                                <option value="pemasukan" {{ old('jenis') === 'pemasukan' ? 'selected' : '' }} class="bg-indigo-950 text-white">Pemasukan</option>
                                <option value="pengeluaran" {{ old('jenis') === 'pengeluaran' ? 'selected' : '' }} class="bg-indigo-950 text-white">Pengeluaran</option>
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="siswa_id" name="siswa_id"
                                class="w-full appearance-none rounded-lg border border-white/30 bg-white/10 px-3 py-2 pr-10 text-sm text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                                <option value="" {{ old('siswa_id') ? '' : 'selected' }} class="bg-indigo-950 text-white">- Tidak terkait siswa -</option>
                                @foreach ($siswas as $siswa)
                                    <option value="{{ $siswa->id }}" {{ old('siswa_id') == $siswa->id ? 'selected' : '' }} class="bg-indigo-950 text-white">{{ $siswa->nis }} - {{ $siswa->nama_lengkap }}</option>
                                @endforeach
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                <svg class="h-4 w-4 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div>
                    ...
                    <textarea id="keterangan" name="keterangan" rows="3"
                        class="w-full rounded-lg border border-white/30 bg-white/10 px-3 py-2 text-sm text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">{{ old('keterangan') }}</textarea>
