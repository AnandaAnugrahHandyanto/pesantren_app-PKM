            ...
            <form action="{{ route('absensi.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select id="mata_pelajaran_id" name="mata_pelajaran_id" required
                        ...
                        <option value="" disabled {{ $mataPelajaranId === '' ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">Pilih mata pelajaran</option>
                        @foreach ($mataPelajaranOptions as $mp)
                            <option value="{{ $mp->id }}" {{ old('mata_pelajaran_id', $mataPelajaranId) == $mp->id ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">
                ...
                <div>
                    ...
                    <select id="siswa_id" name="siswa_id" required aria-label="Pilih siswa"
                        class="w-full rounded-lg border border-slate-400 dark:border-white/30 bg-slate-50 dark:bg-white/10 px-3 py-2 text-sm text-slate-900 dark:text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                        <option value="" disabled selected aria-hidden="true" class="bg-indigo-950 text-slate-900 dark:text-white">Pilih siswa</option>
                        @foreach ($siswas as $siswa)
                            <option value="{{ $siswa->id }}" {{ old('siswa_id') == $siswa->id ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        class="w-full rounded-lg border border-slate-400 dark:border-white/30 bg-slate-50 dark:bg-white/10 px-3 py-2 text-sm text-slate-900 dark:text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                        <option value="" disabled {{ old('status') ? '' : 'selected' }} class="bg-indigo-950 text-slate-900 dark:text-white">Pilih status</option>
                        <option value="hadir" {{ old('status') === 'hadir' ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">Hadir</option>
                        <option value="izin"  {{ old('status') === 'izin'  ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">Izin</option>
                        <option value="sakit" {{ old('status') === 'sakit' ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">Sakit</option>
                        <option value="alfa"  {{ old('status') === 'alfa'  ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">Alfa</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-red-300">{{ $message }}</p>
                    @enderror
