        ...
        <div class="rounded-2xl border border-white/20 bg-white/10 p-6 shadow-lg backdrop-blur-md">
            <form action="{{ route('siswa.update', $siswa) }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            <select id="tingkat" name="tingkat" required aria-label="Pilih tingkat"
                                ...
                                <option value="" disabled class="bg-indigo-950 text-white">Pilih tingkat</option>
                                @foreach ($tingkatOptions as $t)
                                    <option value="{{ $t }}" {{ old('tingkat', $siswa->tingkat) == $t ? 'selected' : '' }} class="bg-indigo-950 text-white">Kelas {{ $t }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        <label for="rombel" class="mb-1 block text-sm font-medium text-white/80">Rombel <span aria-hidden="true">*</span></label>
                                                <div class="relative">
                                                    <select id="rombel" name="rombel" required aria-label="Pilih rombel"
                                                            class="w-full appearance-none rounded-xl border border-white/20 bg-white/10 px-4 py-2.5 pr-10 text-sm text-white backdrop-blur-sm transition focus:border-white/50 focus:outline-none focus:ring-2 focus:ring-white/20">
                                                        <option value="" disabled class="bg-indigo-950 text-white">Pilih rombel</option>
                                                        @foreach ($rombelOptions as $j)
                                                            <option value="{{ $j }}" {{ old('rombel', $siswa->rombel) === $j ? 'selected' : '' }} class="bg-indigo-950 text-white">{{ $j }}</option>
                                                        @endforeach
                                                    </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="jenis_kelamin" name="jenis_kelamin" required aria-required="true" aria-label="Pilih jenis kelamin"
                            class="w-full appearance-none rounded-lg border border-white/30 bg-white/10 px-3 py-2 pr-10 text-sm text-white backdrop-blur-sm focus:border-blue-400/50 focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                            <option value="" disabled aria-hidden="true" class="bg-indigo-950 text-white">Pilih jenis kelamin</option>
                            <option value="L" {{ old('jenis_kelamin', $siswa->jenis_kelamin) === 'L' ? 'selected' : '' }} class="bg-indigo-950 text-white">Laki-laki</option>
                            <option value="P" {{ old('jenis_kelamin', $siswa->jenis_kelamin) === 'P' ? 'selected' : '' }} class="bg-indigo-950 text-white">Perempuan</option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                            <svg class="h-4 w-4 text-white/50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
