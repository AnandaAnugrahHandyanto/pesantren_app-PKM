<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-slate-300 dark:border-white/20 bg-slate-50 dark:bg-white/10 p-4 shadow-lg backdrop-blur-md">
            <form method="GET" action="{{ route('laporan.absensi.semester') }}"
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-xs font-medium text-slate-900 dark:text-slate-600 dark:text-white/70">Tahun Ajaran</label>
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-xs font-medium text-slate-900 dark:text-slate-600 dark:text-white/70">Semester</label>
                    <select name="semester"
                            class="w-full rounded-xl border border-slate-400 dark:border-white/30 bg-slate-50 dark:bg-white/10 px-3 py-2 text-sm text-slate-900 dark:text-white backdrop-blur-sm focus:border-white/50 focus:outline-none focus:ring-2 focus:ring-white/20">
                        <option value="1" {{ $semester === 1 ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">
                            Ganjil (Juli – Desember)
                        </option>
                        <option value="2" {{ $semester === 2 ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">
                            Genap (Januari – Juni)
                        </option>
                    </select>
                </div>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <select name="kelas"
                        ...
                        @foreach ($kelasOptions as $k)
                            <option value="{{ $k }}" {{ request('kelas') == $k ? 'selected' : '' }} class="bg-indigo-950 text-slate-900 dark:text-white">Kelas {{ $k }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex gap-2 sm:items-end">
                    <button type="submit"
                            class="flex-1 rounded-xl bg-indigo-500/80 px-5 py-2 text-sm font-medium text-slate-900 dark:text-white backdrop-blur-sm transition hover:bg-indigo-400/80 sm:flex-none">
                        Tampilkan
                    </button>
                    ...
                    <a href="{{ route('laporan.absensi.cetak', ['semester' => $semester, 'tahun_ajaran' => $tahunAjaran, 'mata_pelajaran_id' => $mataPelajaranId, 'kelas' => request('kelas')]) }}"
                       target="_blank"
                       class="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-300 dark:border-white/20 bg-slate-50 dark:bg-white/10 px-5 py-2 text-sm font-medium text-slate-900 dark:text-slate-700 dark:text-white/80 backdrop-blur-sm transition hover:bg-white/20 sm:flex-none">
        ...
        <p class="text-sm text-slate-900 dark:text-slate-500 dark:text-white/60">
            Menampilkan rekap semester <span class="font-semibold text-slate-900 dark:text-white">{{ $semesterLabel }}</span>
            tahun ajaran <span class="font-semibold text-slate-900 dark:text-white">{{ $tahunAjaran }}/{{ $tahunAjaran + 1 }}</span>
            &mdash;
            periode <span class="font-semibold text-slate-900 dark:text-white">
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-white/10 bg-white dark:bg-white/5">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-900 dark:text-slate-500 dark:text-white/50">Kelas</th>
                ...
                <tbody class="divide-y divide-white/10">
                    @forelse ($rekap as $i => $siswa)
                        <tr class="transition hover:bg-white/[0.04]">
                            ...
                            <td class="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white">{{ $siswa->nama_lengkap }}</td>
                            <td class="px-4 py-3 text-sm text-slate-900 dark:text-slate-600 dark:text-white/70">{{ $siswa->kelas }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-8 text-center text-sm text-slate-900 dark:text-slate-400 dark:text-white/40">
