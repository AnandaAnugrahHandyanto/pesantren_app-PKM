        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex cursor-pointer items-center gap-2">
                <input id="remember_me" type="checkbox"
                    class="rounded border-white/30 bg-white/10 text-indigo-500 shadow-sm focus:ring-2 focus:ring-indigo-400/30 focus:ring-offset-0"
                    name="remember">
                <span class="text-sm text-white/50 hover:text-white/70">{{ __('Remember me') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm text-indigo-300/70 underline decoration-indigo-300/20 underline-offset-2 hover:text-indigo-200" href="{{ route('password.request') }}">
                    {{ __('Lupa password?') }}
                </a>
            @endif
        </div>
